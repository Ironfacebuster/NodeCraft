...
module.exports.execute = (data) => {
    ...
    if (data.arguments.length == 0) data.functions.directMessage(data.user.connection, `${chatColor.aqua("Example command.")} Try again with ${chatColor.green("'random'")} or ${chatColor.blue("'username'")} arguments!`)
    else if (data.arguments[0] == "random") data.functions.directMessage(data.user.connection, `${chatColor.gold("The following value is randomly generated on load:")} ${randomValue}`)
    else if (data.arguments[0] == "username") data.functions.directMessage(data.user.connection, `${chatColor.gray("Your username is:")} ${chatColor.gold(data.player.username)}`)
}
